...
cd ../server && npm ci && npm run build      # compila TypeScript en server/dist
npm run start:prod                          # node dist/index.js, sirve la API + ../web/dist en :8770
```

> En producción se ejecuta el servidor **compilado** (`node dist/index.js`, igual que la imagen Docker), no `tsx` sobre las fuentes. `npm start` sigue siendo el punto de entrada de desarrollo (tsx).
